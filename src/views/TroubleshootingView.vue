...
.trouble-detail { display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; margin: 0; padding: 18px; border: 1px solid var(--glass-border); border-radius: var(--radius-card); background: var(--glass-sheen), var(--glass-bg); backdrop-filter: var(--glass-surface); box-shadow: var(--shadow-glass); }
.trouble-detail div { padding: 4px 14px; border-left: 2px solid rgba(28,32,56,.1); }
...
@media (max-width: 760px) { .trouble-detail { grid-template-columns: 1fr; } .trouble-index { grid-template-columns: 1fr; } }
</style>
